...
def map_line(line):
    ...
    data = line.split(",")
    mapData = []
    mapData.append(int(data[0]))
    mapData.append(int(data[1]))
    mapData.append(float(data[2]))
    return mapData
...
def calculate_loss(pred_matrix, true_matrix):
    ...
    for key in true_matrix:
    	for row in key[1]:
    		i = row[0]
    		j = row[1]
    		y = row[2]
    		p = pred_matrix[i,j]
    		error += math.pow(y - p, 2)
...
def get_worker_id_for_position(word_id, doc_id):
    ...
    row_block = math.ceil((word_id + 1.0) / float(strata_row_size_bc.value))
    return row_block
...
def blockify_matrix(worker_id, partition):
    ...
    for entry in partition:
    	...
    	doc_id = entry[1][1]
    	row_block = worker_id
    	#row_block = math.ceil(word_id / float(strata_row_size_bc.value))
    	col_block = math.ceil((doc_id + 1.0) / float(strata_col_size_bc.value))
    	blocks[col_block, row_block].append(entry[1])
...
def filter_block_for_iteration(num_iteration, col_block_index, row_block_index):
    ...
    num1 = ((col_block_index - row_block_index) % num_workers_bc.value) + 1
    num2 = num_iteration % num_workers_bc.value
    if num2 == 0:
    	num2 = num_workers_bc.value
    return (num1 == num2)
...
def perform_sgd(block):
    ...
    row_start = srs * (row_block - 1) # use col_block and row_block to transform to real row and col indexes
    col_start = scs * (col_block - 1) # use col_block and row_block to transform to real row and col indexes
    ...
    w_gradient = np.zeros(w_mat_block.shape)
    h_gradient = np.zeros(h_mat_block.shape)
    num_updated = 0
    for word_id, doc_id, tfidf_score in tuples:
        ...
        i = word_id % strata_row_size_bc.value
        j = doc_id % strata_col_size_bc.value
        
        w_gradient[i,:] = -2 * (tfidf_score - np.dot(w_mat_block[i,:], h_mat_block.transpose()[:,j])) * h_mat_block[j, :]
        h_gradient[j,:] = -2 * (tfidf_score - np.dot(w_mat_block[i,:], h_mat_block.transpose()[:,j])) * w_mat_block[i, :]
        
    	learning_rate = math.pow(100 + num_old_updates_bc.value, -beta_value_bc.value)
    	if learning_rate < MIN_EPS:
    		learning_rate = MIN_EPS
    		
        # update w_mat_block, h_mat_block
        w_mat_block[i, :] -=  (learning_rate * w_gradient[i,:])
        h_mat_block[j, :] -=  (learning_rate * h_gradient[j,:])
...
if __name__ == '__main__':
    ...
    words = tfidf_scores.map(lambda line: line[0])
    docs = tfidf_scores.map(lambda line: line[1])
    max_word_id = words.max()
    max_doc_id = docs.max()
    ...
    tfidf_local = tfidf_scores.collect()
    ...
    for current_iteration in range(num_iterations):
        ...
        for block_row, block_col, updated_w, updated_h, num_updates in updated:
            ...
            w_update_start = strata_row_size * (block_row - 1)
            w_update_end = strata_row_size * block_row
            w_mat[w_update_start:w_update_end, :] = updated_w

            h_update_start = strata_col_size * (block_col - 1)
            h_update_end = strata_col_size * block_col
            h_mat[h_update_start:h_update_end, :] = updated_h

            num_old_updates += num_updates
            
        calculate_loss(np.dot(w_mat, h_mat.transpose()), tfidf_local)
